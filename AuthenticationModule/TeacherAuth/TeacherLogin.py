...
from AuthenticationModule.TeacherAuth.TeacherRegisteration import RegisterationStart
...
def LoginStart():
    ...
    def SubmitForm():
        ...
        user = Db_Queries.Login_TeacherByEmail(email, Password)
        # print(user[0])
        if user.__len__() != 0:
            UserDet.userDetails = user[0]
            TeacherHomeGUI.start(Toplevel())
        else:
    ...
    # ----------------------------- Registration Form-------------------------
    lbl_0 = Label(frame, text="Teacher Login form", width=40, font=("bold", 36))
